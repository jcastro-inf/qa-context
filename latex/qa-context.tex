...
\begin{enumerate}[(i)]
	...
	\item Build context profile: It analyses the context, which consists on a stream of recent status updates, applies clustering to separate the various topics that the context contains, and generates feature-space profiles for each context topic.
...
\begin{equation}
	profile^{LSA}_u = \sum_{d \in R_u} profile^{LSA}_d = \{ \sum_{d \in R_u} V_{d,1}, \dots, \sum_{d \in R_u} V_{d,f}\}
\end{equation}
