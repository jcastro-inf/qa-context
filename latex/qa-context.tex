...
To include contextual information in the recommendation process, it is needed to build the context profile. In this proposal, the source of the context is a microblogging system that delivers status updates from users. To narrow down the status updates that the system receives in order to focus in an aspect of the context, a set of relevant terms is selected, which contains terms that must appear in the status updates. First, from all the terms that the context contains, LSAContextCluster filters out the terms that do not appear in the set of terms used in the QA site. 
...
The baseline method to compare with is the LSA method without contextual information. For the sake of compareness, 30 features are considered in all LSA models.
...
Moreover, in order to adjust the weight of preference profile over context profile, the methods compared have parameter $\alpha$. In the experiment we explored several values for it, here, to make the results clearer, we show only $\alpha \in [0.85,1.00]$ with increments of $0.01$
...
\begin{figure}[htb]
    \centering
    \includegraphics[width=0.9\textwidth]{figures/ndcg-results-proposal-context-no-clustering.eps}
...
\begin{figure}[htb]
    \centering
    \includegraphics[width=0.9\textwidth]{figures/ndcg-results-proposal-context-fuzzy-clustering-max.eps}
...
\begin{figure}[htb]
    \centering
    \includegraphics[width=0.9\textwidth]{figures/ndcg-results-proposal-context-fuzzy-clustering-membership.eps}
